...
uint8_t RF24N_write(RF24Network * rn, uint16_t to_node, uint8_t directTo)  // Direct To: 0 = First Payload, standard routing, 1=routed payload, 2=directRoute to host, 3=directRoute to Route
{
  uint8_t ok = 0;
  uint8_t isAckType = 0;
  uint32_t reply_time ;
